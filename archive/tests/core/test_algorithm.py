...
from hola.core.params_old import (
...
def test_error_handling(
    ...
    # Non-existent save file should raise FileNotFoundError
...
class TestHOLASaveLoad:
    """Tests for HOLA save and load functionality."""

    def test_save_load_without_config(
        self,
        optimizer: HOLA,
        complex_params: dict[ParamName, ParamConfig],
        dual_objective: dict[ObjectiveName, ObjectiveConfig],
        ...
        """Test saving and loading without configuration data."""
        ...
        # Verify only CSV exists
        assert save_path.with_suffix('.csv').exists()
        assert not save_path.with_suffix('.json').exists()
        ...
        # Verify state
        assert loaded.num_samples == optimizer.num_samples
        for i in range(optimizer.num_samples):
            orig_params, orig_obj = optimizer.get_sample(i)
            loaded_params, loaded_obj = loaded.get_sample(i)
            # Compare parameters with float tolerance
            for key in orig_params:
                assert np.allclose(loaded_params[key], orig_params[key], rtol=1e-10)
            assert loaded_obj == orig_obj
    ...
    def test_error_handling(
        ...
        with pytest.raises(FileNotFoundError, match="Data file not found"):
            HOLA.load(
                nonexistent,
                params_config=optimizer.params,
                objectives_config=optimizer.objectives
            )

        # No config provided should raise ValueError
        with pytest.raises(ValueError, match="Must provide params_config"):
            HOLA.load(nonexistent)

        # load_config=True with missing json should raise ValueError
        with pytest.raises(ValueError, match="Configuration file not found"):
            HOLA.load(nonexistent, load_config=True)

    def test_save_load_parameter_types(
        self,
        dual_objective: dict[ObjectiveName, ObjectiveConfig],
        tmp_path: Path
    ) -> None:
        """Test saving and loading with all parameter types."""
        # Create optimizer with all parameter types
        params_config = {
            ParamName("learning_rate"): create_param_config(
                "continuous",  # Use string instead of enum
                min=1e-4,
                max=1e-1,
                scale="log"
            ),
            ParamName("batch_size"): create_param_config(
                "integer",
                min=16,
                max=256
            ),
            ParamName("optimizer"): create_param_config(
                "categorical",
                categories=["adam", "sgd", "rmsprop"]
            ),
            ParamName("momentum"): create_param_config(
                "lattice",
                min=0.0,
                max=1.0,
                num_values=5
        ...
        hola.add(objectives={"accuracy": 0.90, "time": 120.0}, params=params)

        # Save and load
        save_path = tmp_path / "test_save"
        hola.save(save_path, save_config=True)
        loaded = HOLA.load(save_path, load_config=True)

        # Verify parameter types and values
        assert loaded.num_samples == 1
        loaded_params, _ = loaded.get_sample(0)
