...
from hola.core.sorted_population import TotallyOrderedPopulation, Score, PartiallyOrderedPopulation


class TestScalarSortedPopulation:
    @pytest.fixture
    def empty_population(self) -> TotallyOrderedPopulation[str]:
        return TotallyOrderedPopulation[str]()

    @pytest.fixture
    def populated_population(self) -> TotallyOrderedPopulation[str]:
        pop = TotallyOrderedPopulation[str]()
        pop.add("a", Score(np.array([0.5])))
        pop.add("b", Score(np.array([0.3])))
        pop.add("c", Score(np.array([0.8])))
        return pop

    def test_initialization(self, empty_population: TotallyOrderedPopulation[str]) -> None:
        assert empty_population.num_samples() == 0
        assert empty_population.get_num_fronts() == 0

    def test_add_valid(self, empty_population: TotallyOrderedPopulation[str]) -> None:
    ...
    def test_add_duplicate_label(self, populated_population: TotallyOrderedPopulation[str]) -> None:
        with pytest.raises(ValueError):
            populated_population.add("a", Score(np.array([0.1])))

    def test_add_invalid_score_dimension(
        self, empty_population: TotallyOrderedPopulation[str]
    ) -> None:
        with pytest.raises(ValueError):
            empty_population.add("test", Score(np.array([0.5, 0.3])))

    def test_get_score_missing_label(self, empty_population: TotallyOrderedPopulation[str]) -> None:
        with pytest.raises(KeyError):
            empty_population.get_score("missing")

    def test_update_score(self, populated_population: TotallyOrderedPopulation[str]) -> None:
    ...
    def test_sorted_labels(self, populated_population: TotallyOrderedPopulation[str]) -> None:
        assert populated_population.get_sorted_labels() == ["b", "a", "c"]

    def test_front_access(self, populated_population: TotallyOrderedPopulation[str]) -> None:
    ...
    def test_front_index(self, populated_population: TotallyOrderedPopulation[str]) -> None:
    ...
    def test_crowding_distance(self, populated_population: TotallyOrderedPopulation[str]) -> None:
    ...
    def test_get_top_samples(self, populated_population: TotallyOrderedPopulation[str]) -> None:
...
class TestVectorSortedPopulation:
    @pytest.fixture
    def empty_population(self) -> PartiallyOrderedPopulation[str]:
        return PartiallyOrderedPopulation[str]()

    @pytest.fixture
    def simple_population(self) -> PartiallyOrderedPopulation[str]:
        """Population with clear dominance relationships."""
        pop = PartiallyOrderedPopulation[str]()
        # a dominates b
    ...
    @pytest.fixture
    def pareto_population(self) -> PartiallyOrderedPopulation[str]:
    ...
    @pytest.fixture
    def complex_pareto_population(self) -> PartiallyOrderedPopulation[str]:
    ...
    def test_initialization(self, empty_population: PartiallyOrderedPopulation[str]) -> None:
        assert empty_population.num_samples() == 0
        assert empty_population.get_num_fronts() == 0

    def test_add_valid(self, empty_population: PartiallyOrderedPopulation[str]) -> None:
        empty_population.add("test", Score(np.array([0.5, 0.3])))
        assert empty_population.num_samples() == 1
        assert_array_equal(empty_population.get_score("test"), np.array([0.5, 0.3]))

    def test_add_duplicate_label(self, simple_population: PartiallyOrderedPopulation[str]) -> None:
        with pytest.raises(ValueError):
            simple_population.add("a", Score(np.array([0.1, 0.1])))

    def test_add_inconsistent_dimensions(
        self, simple_population: PartiallyOrderedPopulation[str]
    ) -> None:
        with pytest.raises(ValueError):
            simple_population.add("c", Score(np.array([0.1, 0.1, 0.1])))

    def test_dominates(self, simple_population: PartiallyOrderedPopulation[str]) -> None:
    ...
    def test_front_assignment_simple(self, simple_population: PartiallyOrderedPopulation[str]) -> None:
        assert simple_population.get_front(0) == ["a"]
        assert simple_population.get_front(1) == ["b"]
        assert simple_population.get_num_fronts() == 2

    def test_front_assignment_pareto(self, pareto_population: PartiallyOrderedPopulation[str]) -> None:
        front_0 = set(pareto_population.get_front(0))
        assert front_0 == {"a", "b"}
        assert pareto_population.get_front(1) == ["c"]
        assert pareto_population.get_num_fronts() == 2

    def test_front_index(self, pareto_population: PartiallyOrderedPopulation[str]) -> None:
    ...
    def test_crowding_distance(self, pareto_population: PartiallyOrderedPopulation[str]) -> None:
    ...
    def test_complex_front_structure(
        self, complex_pareto_population: PartiallyOrderedPopulation[str]
    ) -> None:
    ...
    def test_score_update_front_changes(
        self, pareto_population: PartiallyOrderedPopulation[str]
    ) -> None:
    ...
    def test_get_sorted_labels(self, pareto_population: PartiallyOrderedPopulation[str]) -> None:
