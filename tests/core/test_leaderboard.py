...
class TestTrial:
    def test_valid_trial(self) -> None:
        trial = Trial(params={ParamName("lr"): 0.001}, objectives={ObjectiveName("acc"): 0.95})
        assert trial.params[ParamName("lr")] == 0.001
...
class TestLeaderboard:
    ...
    def populated_leaderboard(self, simple_scorer: ObjectiveScorer) -> Leaderboard:
        ...
        board.add(params={ParamName("lr"): 0.001}, objectives={ObjectiveName("accuracy"): 0.90})
        board.add(params={ParamName("lr"): 0.01}, objectives={ObjectiveName("accuracy"): 0.85})
        return board
    ...
    def test_get_sample(self, populated_leaderboard: Leaderboard) -> None:
        ...
        assert params[ParamName("lr")] == 0.001
    ...
    def test_get_best_sample_single_objective(self, populated_leaderboard: Leaderboard) -> None:
        ...
        assert params[ParamName("lr")] == 0.001  # Higher accuracy is better
        assert objectives[ObjectiveName("accuracy")] == 0.90
    ...
    def test_add_missing_objective(self, simple_scorer: ObjectiveScorer) -> None:
        ...
        with pytest.raises(KeyError):
            board.add(params={ParamName("lr"): 0.001}, objectives={ObjectiveName("invalid"): 0.90})
