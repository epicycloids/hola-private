...
from hola.core.objective import ObjectiveConfig, ObjectiveName, ObjectiveScorer, create_objective
...
class Trial(BaseConfig):
    ...
    """
    params: dict[ParamName, float] = Field(
        ...,
...
class Leaderboard:
    ...
    scores from objective values, and caches parameter settings with their resulting
    objective values. It supports both single-objective and multi-objective
    optimization through appropriate population sorting strategies.
    ...
    def create_single_objective(
        ...
        return cls(ObjectiveScorer({name: obj_config}))

    @classmethod
    def create_dual_objective(
        cls,
        primary_name: str,
        primary_config: dict[str, Any] | str | bytes,
        secondary_name: str,
        secondary_config: dict[str, Any] | str | bytes,
        ...
        return cls(ObjectiveScorer(objectives))
    ...
    def create_pareto(
        ...
        return cls(ObjectiveScorer(configs))
    ...
    def __init__(self, objective_scorer: ObjectiveScorer):
        ...
        self.sorted_population = (
            VectorSortedPopulation[int]()
            if self.objective_scorer.multigroup
            else ScalarSortedPopulation[int]()
        )
    ...
    def filter_samples(self, keep_indices: list[int]) -> None:
        ...
        if self.objective_scorer.multigroup:
            new_population = VectorSortedPopulation[int]()
        else:
            new_population = ScalarSortedPopulation[int]()
    ...
    def get_best_sample(self) -> tuple[dict[ParamName, float], dict[ObjectiveName, float]]:
        ...
        Example:
            >>> params, objectives = leaderboard.get_best_sample()
            >>> print(f"Best accuracy: {objectives['accuracy']:.2f}")
    ...
    def get_top_samples(
        ...
        Example:
            ...
            >>> top_5 = leaderboard.get_top_samples(5)
            >>> for params, objectives in top_5:
            ...     print(f"Accuracy: {objectives['accuracy']:.2f}")
    ...
    def get_dataframe(self) -> pd.DataFrame:
        ...
        Example:
            ...
            >>> print("Top accuracy:", df["objective_accuracy"].max())
    ...
    def save(self, filename: Path | str) -> None:
        ...
        Example:
            ...
            >>> leaderboard.save("optimization_results.csv")
            >>> # Share results
            >>> leaderboard.save(Path("shared/team_results.csv"))
    ...
    def load(cls, filename: Path | str, objective_scorer: ObjectiveScorer) -> Leaderboard:
        ...
        Example:
            ...
            ...     objective_scorer
            ... )
            >>> print(f"Loaded {leaderboard.num_samples()} samples")
