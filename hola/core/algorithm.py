...
from hola.core.params import ParamInfeasibleError, ParamName, ParamConfig, ParameterTransformer
from hola.core.sample import MixtureSampler, SampleType
...
class HOLA(BaseModel):
    ...
    params_config: dict[ParamName, SkipValidation[ParamConfig]]
    objectives_config: dict[ObjectiveName, SkipValidation[ObjectiveConfig]]

    # Optional configuration with validation
    top_fraction: float = Field(gt=0, lt=1, default=0.2)
    min_samples: int | None = Field(gt=0, default=None)
    min_fit_samples: int | None = Field(gt=0, default=None)
    n_components: int = Field(gt=0, default=3)
    gmm_reg: float = Field(ge=0, default=0.0005)
    gmm_sampler: SampleType = Field(default=SampleType.UNIFORM)
    explore_sampler: SampleType = Field(default=SampleType.SOBOL)
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        self._param_transformer = ParameterTransformer(self.params_config)
        self._objective_scorer = ObjectiveScorer(self.objectives_config)
    ...
    def update_params_config(
        ...
        new_config = self.params_config.copy()
        new_config.update(updates)

        # Verify all parameters being updated exist
        invalid_params = set(updates.keys()) - set(self.params_config.keys())
        if invalid_params:
            ...
            try:
                ...
                if storage_mode == SampleStorageMode.ARCHIVE:
                    self._archived_trials.append(ArchivedTrial(
                        ...
                        param_config=self.params_config
                    ))

        # Update state
        self.params_config = new_config
    ...
    def update_objectives_config(
        ...
        """
        # Create new merged config
        new_config = self.objectives_config.copy()
        new_config.update(updates)
        ...
        # Update state
        self.objectives_config = new_config
    ...
    def add(
        ...
        invalid_params = set(params.keys()) - set(self.params_config.keys())
        if invalid_params:
            raise KeyError(f"Invalid parameter names: {invalid_params}")

        # Validate objective names
        invalid_objectives = set(objectives.keys()) - set(self.objectives_config.keys())
    ...
    def save(self, filename: Path | str) -> None:
        ...
        """
        self._leaderboard.save(filename)
    ...
    def load(self, filename: Path | str) -> None:
        ...
        :type filename: Path | str
        """
        self._leaderboard = Leaderboard.load(filename, self._objective_scorer)
        num_samples = self.num_samples
        # Burn samples to correctly set the sampler (do this instead of
        # adjusting the sampler since we need to go through the Sobol'
        # sequence)
        for _ in range(num_samples):
            self._mixture_sampler.sample()
        self._update_sampler()
    ...
    def __repr__(self) -> str:
        ...
        return (
            ...
            f"num_params={len(self.params_config)}, "
            f"num_objectives={len(self.objectives_config)}"
            ")"
        )